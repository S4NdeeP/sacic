...
from misc.densenet_utils import myDensenet
import misc.densenet as densenet

def main(params):
  net = getattr(densenet, params['model'])()
  #net.my_features = nn.Sequential(*list(net.features.children())[:-1])
  ...
  model = nn.Sequential(*list(net.features.children())[:-1])
  ...
  for i,img in enumerate(imgs):
    ...
    #if(os.path.isfile(os.path.join(dir_att, str(img['cocoid'])+ '.npz'))):
    #  continue
    print(os.path.join(dir_att, str(img['cocoid'])))
    ...
    #print(np.shape(I))
    I = trn.functional.to_pil_image(I)
    I = preprocess(I)
    I = I.cuda() 
    with torch.no_grad():
      tmp_att = my_densenet(I, params['att_size'])
    # write to pkl
    #np.save(os.path.join(dir_fc, str(img['cocoid'])), tmp_fc.data.cpu().float().numpy())
    #print("To Save")
    #print(np.shape(tmp_att.data.cpu().float().numpy()))
...
if __name__ == "__main__":
  ...
  parser.add_argument('--model', default='densenet201', type=str, help='densenet121, densenet169, densenet201, densenet161')
